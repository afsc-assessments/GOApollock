...
```{r catches, eval=TRUE} 
 ...
 flextable(x$tab) %>%   
       set_caption(x$cap) %>%
 colformat_double(digits=0) %>% fontsize(size=7)
``` 
 
\pagebreak

```{r incidental, eval=TRUE} 
 x <- gettab(sheet = 'incidental', maxcols = 6) 
 flextable(x$tab) %>%   
...
```{r non-commercial} 
 x <- gettab(sheet = 'non_commercial', maxcols=4) 
 flextable(x$tab) %>%   
       set_caption(x$cap) %>%
...
```{r waa-srv26} 
 waa_srv2 <- datfile$wt_srv2 %>% data.frame(ayears,.) %>% 
 	setNames(c('Year',1:10)) %>% 
        subset(Year %in% datfile$srv_acyrs2 & Year >=1984) 
      cap <-'Weight at age (kg) of pollock in the summer NMFS bottom trawl survey (top) and NMFS summer acoustic survey (bottom).' 
 flextable(waa_srv2) %>%   
       set_caption(cap)  %>%  
 	  colformat_double(digits=3)%>% 
 	   colformat_num(digits=0, big.mark='', j=1) %>% 
 ...
 waa_srv6 <- datfile$wt_srv6 %>% data.frame(ayears,.) %>% 
 	setNames(c('Year',1:10)) %>% 
        subset(Year %in% datfile$srv_acyrs6) 
 flextable(waa_srv6) %>%   
 	  colformat_double(digits=3)%>% 
 	   colformat_num(digits=0, big.mark='', j=1) %>% 
  	fontsize(size=9) 
...
```{r selex} 
  ...
  s6[1:2] <- c(repfile$q4, repfile$q5) 
  s7 <- repfile$slctsrv6 
  s8 <- repfile$slctsrv2 
  s9 <- repfile$slctsrv3 
...
```{r naa-est}
 naa <- 1e3*repfile$N %>% as.data.frame() %>% 
  setNames(1:10) %>% 
...
```{r timeseries}
sumbio <- repfile$Esumbio*1e3
ssb <- repfile$Espawnbio*1e3
pct.ssb <- paste0(round(100*ssb/(Finv(b100)/1000),0),"%")
rec <- repfile$recruit*1e3
catch <- repfile$cattot
hr <- paste0(round(100*catch/(1000*sumbio),0), '%')
sumbio2 <- c( repfile.last$Esumbio*1e3, NA)
ssb2 <- c(repfile.last$Espawnbio*1e3, NA)
rec2 <- c(repfile.last$recruit*1e3,NA)
...
tmpyrs <- c("",paste0(year," assessment"), paste0(year-1," assessment"))
...
flextable(x$tab) %>%   
      set_caption(x$cap) %>% 
	  colformat_double(digits=0, big.mark='',j=1) %>%
	  colformat_double(digits=0, big.mark=',',j=4) %>% 
  fontsize(size=8) %>%
  width( j=2, width=2.5)
``` 
...
tmp <- proj_scens
tmp <- dplyr::select(proj_scens, Year, Alt, SSB, F, Catch) %>%
  dplyr::rename(Year=Year, alt=Alt)
