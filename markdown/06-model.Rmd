...
(`r length(datfile$years)` years) was used to assess Gulf of Alaska
...
survey and the bottom trawl were given an initial sample size of
60, and the ADF&G crab/groundfish survey was given a weight of
30.
...
different approaches. @merati1993 noted that ovaries classified
...
backscatter by within that stratum into abundance. To generate
...
pollock abundances in each stratum scaled by dividing by the mean
...
variable was the age, and data from each haul weighted by the
...
1.\@ref(fig:fig-mat)). Inter-annual changes in maturity at age
...
1.\@ref(tab:waa-srv2). Data from the Shelikof Strait acoustic
...
details are provided in @ianelli2016. Input data included fishery
weight age for 1975-`r year`. The model also incorporates survey
...
age. A comparison of RE model estimates from last year of the `r
year-1` fishery weight at age with the data now available
indicate that the model underestimated weights slightly
(Fig. 1.\@ref(fig:waa-fsh)). In this assessment, RE model estimates of
