...
// This model was modified to better match WHAM for a bridging exercise:
// - assume constant fraction year for surveys (hacked code)
// - add some new reports and adreport for log qs
// - switch recdevs to normal vector so can modify it
// - switch from CV to dnorm(,sd) statements
// - add sigmaR to all recdev pars
// - turn off length comp contributions
// - turn off ageing error matrix (set to identity)
// - change "o" value in age comp calcs
// - set ac_yng_fsh to 1 manually
// - turn off age accumulation in survey 1
// -turn off normalizing selex curves 
// - manually fix TV q random walk SDs to be constant
...
//Fishery 
  ...
  !! for(int i=1; i<=nyrs_fsh;i++) ac_yng_fsh(i)= ac_yng_fsh(i)-1;
...
//EK500
  ...
  !! for(int i=styr; i<=endyr-1; i++){ q1_rwlk_sd(i)=0.038;}
...
//Survey 3 (ADFG coastal survey)
  ...
  !! for(int i=styr; i<=endyr-1; i++){ q3_rwlk_sd(i)=0.05;}
...
//  init_bounded_number M(0.1,0.5,-1)
  ...
  init_bounded_vector dev_log_recruit(styr,endyr,-15,15,3)
...
//Fishing mortality and survey catchablility
  ...
  init_bounded_dev_vector log_q1_dev(styr,endyr,-5,5,4) 
...
//Dependent parameters
  ...
  sdreport_vector log_ssb(styr,endyr) 
...
// turn off ageing error matrix for bridging test
    for(int i=rcrage; i<=trmage; i++){
      for(int j=rcrage; j<=trmage; j++){
	if(i==j) age_trans(i,j)=1.0; else age_trans(i,j)=0.0;
    }
  }
...
// Catch at age and survey numbers at age
  ...
  for (j=rcrage;j<=trmage;j++)
    ...
    Nsrv1(i,j)=slctsrv1(j)*N(i,j)*mfexp(-yrfrct_srv1(styr)*Z(i,j));
    Nsrv2(i,j)=slctsrv2(j)*N(i,j)*mfexp(-yrfrct_srv2(styr)*Z(i,j));
    Nsrv3(i,j)=slctsrv3(j)*N(i,j)*mfexp(-yrfrct_srv3(styr)*Z(i,j));
...
FUNCTION Expected_values
  ...
  for (i=styr;i<=endyr;i++)
    ...
    Eindxsurv1_bs(i)= q1_bs*sum(elem_prod(elem_prod(elem_prod(N(i),mfexp(-yrfrct_srv1(styr)*Z(i))),slctsrv1),wt_srv1(i)));
    Eindxsurv1(i)= q1(i)*sum(elem_prod(elem_prod(elem_prod(N(i),mfexp(-yrfrct_srv1(styr)*Z(i))),slctsrv1),wt_srv1(i)));
    Esrvp1(i) = (Nsrv1(i)/sum(Nsrv1(i)))*age_trans;
    Esrvlenp1(i) = Esrvp1(i) * len_trans3;

    Eindxsurv2(i)= q2(i)*sum(elem_prod(elem_prod(elem_prod(N(i),mfexp(-yrfrct_srv2(styr)*Z(i))),slctsrv2),wt_srv2(i)));
    Esrvp2(i) = (Nsrv2(i)/sum(Nsrv2(i)))*age_trans;
    Esrvlenp2(i) = Esrvp2(i) * len_trans2;

    Eindxsurv3(i)= q3(i)*sum(elem_prod(elem_prod(elem_prod(N(i),mfexp(-yrfrct_srv3(styr)*Z(i))),slctsrv3),wt_srv3(i)));
   ...
   Eindxsurv6(i)= q6*sum(elem_prod(elem_prod(elem_prod(N(i),mfexp(-yrfrct_srv6(styr)*Z(i))),slctsrv6),wt_srv6(i)));
...
// 2+ biomass at spawning (use for apportioning to management area)
    Espawnbio_2plus(i)= sum(elem_prod(elem_prod(N(i)(rcrage+1,trmage),mfexp(-yrfrct_srv1(styr)*Z(i)(rcrage+1,trmage))),wt_srv1(i)(rcrage+1,trmage)));
...
// Survey 2
  ...
  for (j=rcrage;j<=trmage;j++)
    ...
    if(j<ac_yng_srv6(i)) 
      {
      Esrvp6(srv_acyrs6(i),ac_yng_srv6(i)) += Esrvp6(srv_acyrs6(i),j);
...
//Total catch
 dvar_vector cattot_log_sd2=sqrt(log(square(cattot_log_sd)+1.0));	  
  loglik(1) = -.5*norm2(elem_div((log(cattot)-log(Ecattot)),cattot_log_sd));	  
  loglik(1) = -.5*norm2(elem_div((log(cattot)-log(Ecattot)),cattot_log_sd2));
//Age composition
  for (i=1;i<=nyrs_fsh;i++) {
    llcatp(i) =  0;
...
//       (log(indxsurv1_bs)-log(Eindxsurv1_bs(srvyrs1_bs))+square(indxsurv_log_sd1_bs)/2.),indxsurv_log_sd1_bs));
 ...
 loglik(4) = -.5*norm2(elem_div(
      (log(indxsurv1)-log(Eindxsurv1(srvyrs1))+square(indxsurv_log_sd1)/2.),
      indxsurv_log_sd1));
  // loglik(4) = -.5*norm2(elem_div(
  // (log(indxsurv1)-log(Eindxsurv1(srvyrs1))),
  // sqrt(log(square(indxsurv_log_sd1)+1.0))));

   dvar_vector sd1=sqrt(log(square(indxsurv_log_sd1)+1.0));	  
   loglik(4) = -.5*norm2(elem_div(log(indxsurv1)-log(Eindxsurv1(srvyrs1)), sd1));	  
...
//Age composition
 ...
 //    for (j=ac_yng_srv1(i);j<=ac_old_srv1(i);j++)
    for (j=ac_yng_srv1(i);j<=ac_old_srv1(i);j++)
    {
    // check that this is not double counting the age 1 and age 2 fish, should be zeroes so should be ok
      // if(srvp1(i,j)>0)
...
//Total biomass    
  loglik(7) = -.5*norm2(elem_div(
   ...
   dvar_vector sd2=sqrt(log(square(indxsurv_log_sd2)+1.0));	  
   loglik(7) = -.5*norm2(elem_div(log(indxsurv2)-log(Eindxsurv2(srvyrs2)), sd2));	  
...
//Total biomass
   ...
   dvar_vector sd3=sqrt(log(square(indxsurv_log_sd3)+1.0));	  
   loglik(11) = -.5*norm2(elem_div(log(indxsurv3)-log(Eindxsurv3(srvyrs3)), sd3));	  
...
// Survey 4 and 5 likelihoods
   ...
   loglik(14) = -.5*norm2(elem_div((log(indxsurv4)-log(Eindxsurv4(srvyrs4))+square(indxsurv_log_sd4)/2.),indxsurv_log_sd4));
    dvar_vector sd4=sqrt(log(square(indxsurv_log_sd4)+1.0));	  
   loglik(14) = -.5*norm2(elem_div(log(indxsurv4)-log(Eindxsurv4(srvyrs4)), sd4));	  
  RMSE_srv4= sqrt(norm2(log(indxsurv4)-log(Eindxsurv4(srvyrs4))+square(indxsurv_log_sd4)/2.)/nyrs_srv4);
   ...
   loglik(15) = -.5*norm2(elem_div((log(indxsurv5)-log(Eindxsurv5(srvyrs5))+square(indxsurv_log_sd5)/2.),indxsurv_log_sd5));
    dvar_vector sd5=sqrt(log(square(indxsurv_log_sd5)+1.0));	  
   loglik(15) = -.5*norm2(elem_div(log(indxsurv5)-log(Eindxsurv5(srvyrs5)), sd5));	  
...
//Total biomass    
  loglik(16) = -.5*norm2(elem_div(
    ...
    dvar_vector sd6=sqrt(log(square(indxsurv_log_sd6)+1.0));	  
   loglik(16) = -.5*norm2(elem_div(log(indxsurv6)-log(Eindxsurv6(srvyrs6)), sd6));	  
...
//  loglik(18) += -0.5*square(dev_log_recruit(endyr)/0.25);
 // This one matches WHAM exactly b/c first year recruit is not
 // evalulated and instead is part of initial age structure
  loglik(18)= dnorm(dev_log_recruit(styr+1,endyr),0.0, 1.0);
  // this one is close to wham
  loglik(18) = -0.5*norm2(dev_log_recruit);
...
//  report << RMSE_srv1_bs << endl;
  ...
  report <<     F_proj << endl;  
  report << "ageing error matrix" << endl;;
