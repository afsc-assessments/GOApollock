...
library(flextable)
# globals
figdir <- function(x) file.path('C:/Users/cole.monnahan/Work/assessments/GOA_pollock/2021/writeup/figures/',x)
gettab <- function(sheet, ...){
  p <- 'C:/Users/cole.monnahan/Work/assessments/GOA_pollock/2022/writeup/2022_GOApollock tables.xlsx'
  stopifnot(file.exists(p))
  cap <- readxl::read_xlsx(path=p, sheet=sheet, col_names=FALSE, range='A1', col_types = 'text')
  cap <- as.character(cap[1,1]) # ditch merged cell stuff
  tab <- readxl::read_xlsx(path=p, sheet=sheet, skip=1, ...)
