...
```{r mod-compare, eval=TRUE, fig.cap="Estimated SSB for the previous (19.1) and new (19.1a) models run with the 2021 and 2022 data."}
...
addfig("mod_compare2.png")
```


```{r tv-q, eval=TRUE, fig.cap="Time-varying catchability for the Shelikof Strait acoustic survey (Survey 1), the ADF\\&G crab/groundfish trawl survey (Survey 3), and constant catchability for the age-1 and age-2 Shelikof indices (Surveys 4 and 5; representing selectivity), and for the summer NMFS acoustic survey (Survey 6), for model 19.1a. Ribbons and lines represent the 95% CI"}
 addfig("tv_q.png")
```
...
```{r fig-selex, eval=TRUE, fig.cap="Model predicted (line) and observed survey biomass (points and 95% confidence intervals)  for the age 1 and age 2 winter Shelikof surveys. "}
...
```{r retros, eval=TRUE, fig.cap=paste0("Retrospective plot of spawning biomass for models ending in years ",year-9,"-",year-1, " for the ",year," base model. The revised Mohn’s rho (Mohn 1999) for ending year spawning biomass is -0.051.")}
 addfig("retros.png")
```
